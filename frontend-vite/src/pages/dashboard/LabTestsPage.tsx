...
export function LabTestsPage() {
  ...
  const { register, handleSubmit, watch, formState: { errors, isSubmitting } } = useForm<LabTestFormValues>({
  ...
  const onSubmit = async (_data: LabTestFormValues) => {
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="rounded-lg bg-green-50 p-6 text-center">
              <h3 className="text-lg font-semibold text-gray-900">Lab Test Request Submitted</h3>
              <p className="mt-2 text-sm text-gray-600">In production, a phlebotomist would be scheduled. This is a mock confirmation.</p>
              <Button className="mt-4" onClick={() => setSubmitted(false)} variant="outline">Book another test</Button>
            </div>
          </CardContent>
        </Card>
      </motion.div>
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-2xl font-bold text-gray-900">Book Lab Test at Home</h2>
        <p className="mt-1 text-gray-600">Select test type, enter address, and choose a time slot. Location is mocked.</p>
      </motion.div>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <Input id="address" placeholder="Street, city, postal code" {...register("address")} className="mt-1" />
              {errors.address && <p className="mt-1 text-sm text-red-600">{errors.address.message}</p>}
            </div>
            <div>
              <Label htmlFor="preferredTime">Preferred time slot</Label>
              <Select id="preferredTime" {...register("preferredTime")} className="mt-1">
            ...
            {selectedTest && (
              <p className="text-sm text-gray-600">Test: <strong>{selectedTest.name}</strong> — ${selectedTest.price}</p>
            )}
