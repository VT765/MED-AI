...
export function DoctorListCard({ doctor }: { doctor: Doctor }) {
  return (
    <Link to={`/dashboard/appointment/doctors/${doctor.id}`} className="block">
        ...
        <CardContent className="p-0">
          <div className="relative pt-6">
            <div className="mx-auto flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-gray-200 bg-primary-50 text-primary-600">
              <span className="text-2xl font-semibold">{doctor.name.split(" ").map((n) => n[0]).join("")}</span>
            </div>
          ...
          <div className="p-4 text-center">
            ...
            <div className="mt-2 inline-flex rounded-full bg-primary-100 px-2.5 py-0.5 text-xs font-semibold text-primary-700">${doctor.fee} consultation</div>
          </div>
        </CardContent>
      </Card>
    </Link>
