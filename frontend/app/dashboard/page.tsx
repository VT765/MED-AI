...
const modules = [
  ...
  },
  {
    ...
    title: "Personalized Healthcare Plan",
    description: "AI fitness & diet plans tailored to you.",
    icon: Activity,
    color: "bg-amber-100 text-amber-700",
    disabled: true,
    badge: "Coming Soon",
    tooltip: "Personalized AI fitness and diet plans based on your health profile. Coming soon.",
  },
];
          ...
          const cardContent = (
            <Card
              className={cn(
                ...
                isComingSoon
                  ? "cursor-not-allowed border-stone-200 bg-surface-muted/80 opacity-90"
                  : "hover:border-primary-200 hover:shadow-cardHover"
              )}
            >
              <CardHeader>
                <div className="flex items-start justify-between gap-2">
                  <div
                    className={cn(
                      ...
                      isComingSoon ? "bg-amber-100 text-amber-700" : module.color
                    )}
                  >
                    {isComingSoon ? (
                  ...
                  {isComingSoon && (
                    <Badge variant="comingSoon">{module.badge}</Badge>
                  )}
                </div>
          ...
          if (isComingSoon) {
            return (
              <motion.div
                ...
                className="relative"
                onMouseEnter={() => setHoveredComingSoon(true)}
                onMouseLeave={() => setHoveredComingSoon(false)}
              >
                <div className="h-full" title={module.tooltip}>
                  {cardContent}
                </div>
                {hoveredComingSoon && (
                  <div
                    role="tooltip"
                    className="absolute bottom-full left-1/2 z-50 mb-2 max-w-xs -translate-x-1/2 rounded-input border border-stone-200 bg-content-primary px-3 py-2 text-xs text-white shadow-cardHover"
                  >
                    {module.tooltip}
                  </div>
                )}
