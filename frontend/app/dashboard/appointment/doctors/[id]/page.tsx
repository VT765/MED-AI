...
export default function DoctorBiographyPage() {
  ...
  const profile = getDoctorProfile(id);

  if (!profile) {
    return (
      <div className="py-12 text-center">
        <p className="text-gray-500">Doctor not found.</p>
        <Link href="/dashboard/appointment" className="mt-4 inline-block text-primary-600 hover:underline">
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      {/* Header: back + "Doctor's Biography" */}
      <motion.div
      ...
      {/* Primary profile card: circular photo, flag, name, specialization, location, years, consultation fees */}
      <Card>
              ...
              <div className="mt-4">
                ...
                <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-600">
                  <span className="flex items-center gap-2">
                    ...
                    Video consultation: ${profile.videoFee}
                  </span>
                  <span className="flex items-center gap-2">
                    <MessageSquare className="h-4 w-4" aria-hidden />
                    Written consultation: ${profile.writtenFee}
                  </span>
                </div>
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div>
            <h4 className="text-sm font-medium text-gray-500">Subspecialty</h4>
            <p className="text-gray-900">{profile.subspecialty}</p>
          </div>
          <div>
            <h4 className="text-sm font-medium text-gray-500">Diseases</h4>
            <p className="text-gray-900">{profile.diseases.join(", ")}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          ...
          <Link
            href={`/dashboard/appointment/doctors/${id}/book`}
            className="inline-flex h-10 items-center justify-center rounded-xl bg-teal-600 px-4 py-2 text-sm font-medium text-white hover:bg-teal-700"
          >
        ...
        <CardContent>
          <ul className="list-disc space-y-2 pl-5 text-sm text-gray-700">
            {profile.experienceList.map((exp, i) => (
              <li key={i}>
                {exp.title}, {exp.institution}, {exp.location}. {exp.period}
              </li>
            ))}
          </ul>
          <button type="button" className="mt-4 flex items-center gap-1 text-sm font-medium text-primary-600 hover:underline">
            Show more <ChevronDown className="h-4 w-4" aria-hidden />
          </button>
        </CardContent>
      </Card>
