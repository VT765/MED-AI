...
export function EmergencyForm() {
        ...
        <CardContent className="pt-6">
          <div className="rounded-lg bg-white p-6 text-center shadow-soft">
            ...
            <h3 className="text-lg font-semibold text-gray-900">
              Transport Booking Request Received
            </h3>
            <p className="mt-2 text-sm text-gray-600">
              Your inter-city patient transport request has been submitted. 
              Our team will contact you shortly to confirm the details and schedule.
            </p>
            <p className="mt-4 text-sm font-medium text-gray-700">
              For urgent medical emergencies, please call local emergency services (e.g. 911) immediately.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="border-primary-200 bg-white">
      <CardHeader>
        <h3 className="text-lg font-semibold text-gray-900">
          Inter-City Patient Transport Booking
        </h3>
        <p className="text-sm text-gray-500">
          Fill in the details to book ambulance transport between cities. 
          Our professional medical transport service ensures safe patient transfer.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              id="transportReason"
              placeholder="Describe the reason for transport (e.g. hospital discharge, scheduled appointment, medical transfer)"
              rows={4}
              {...register("transportReason")}
              className="mt-1"
            />
            {errors.transportReason && (
              <p className="mt-1 text-sm text-red-600">
                {errors.transportReason.message}
              </p>
            )}
          </div>
          <Button
